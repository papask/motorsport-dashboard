...
## v5 — `onthelimit-promo-v5.mp4` (최신 · 90초)

![poster](poster-v5.jpg)

v4에서 **오프닝만 바꾼** 버전입니다. 나머지 장면, 음악, 효과음, 길이(정확히 90.00초)는 v4와 같습니다.

- **오프닝(0:00–0:03):** 타이밍 타워와 선수 정보(번호, 이름, 팀)를 빼고, **텔레메트리만** 화면 전체에 크게 보여 줍니다.
  - 표시 요소: 시프트 라이트, 속도, 기어, RPM, 스로틀·브레이크, DRS, 랩 전체의 속도 그래프
  - 데이터는 스페인 GP 57랩 최속랩의 가장 긴 풀스로틀 직선입니다. 3단에서 8단까지 실제 변속 시점마다 변속음이 나고, 리미터에서 시프트 라이트가 파랗게 깜빡이며 **ON THE LIMIT**이 뜹니다.
- **다시 만드는 방법:** v4 준비를 마친 뒤 `python3 scripts/v5/make_video5.py`를 실행합니다. `make_video4.py`를 가져다 쓰면서 오프닝 화면만 바꿉니다.

---

## v4 — `onthelimit-promo-v4.mp4` (90초)
