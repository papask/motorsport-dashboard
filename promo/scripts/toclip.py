...
os.makedirs(S+'/clips',exist_ok=True)
for d in sorted(os.listdir(S+'/rec')):
    p=f'{S}/rec/{d}'
    ...
    subprocess.run([FF,'-y','-loglevel','error','-f','concat','-safe','0','-i',p+'/list.txt','-vf',f'fps=30,scale={w}:-2:flags=lanczos,format=yuv420p','-c:v','libx264','-crf','12','-preset','fast',f'{S}/clips/{d}.mp4'],check=True)
