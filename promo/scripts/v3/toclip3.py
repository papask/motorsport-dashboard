...
os.makedirs(S+'/clips3',exist_ok=True)
for d in sorted(os.listdir(S+'/rec3')):
    p=f'{S}/rec3/{d}'
    ...
    w=1920 if d.startswith('w') else 780
    subprocess.run([FF,'-y','-loglevel','error','-f','concat','-safe','0','-i',p+'/list.txt','-vf',f'fps=30,scale={w}:-2:flags=lanczos,format=yuv420p','-c:v','libx264','-crf','12','-preset','fast',f'{S}/clips3/{d}.mp4'],check=True)
