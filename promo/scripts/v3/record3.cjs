...
const OUT = path.join(__dirname, 'rec3');
...
const mobileScenes = {
  ...
  async m02_schedule(p) {
    ...
    await wait(p, 700);
    await tap(p, seg(p, '전 세션')); await wait(p, 1200);
    await swipe(p, 900, 2200); await wait(p, 700);
    await swipe(p, 900, 1800); await wait(p, 600);
    await stop();
  },
