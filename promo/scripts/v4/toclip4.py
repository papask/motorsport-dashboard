...
os.makedirs(S+'/clips4',exist_ok=True)
for d in sorted(os.listdir(S+'/rec4')):
    p=f'{S}/rec4/{d}'
    ...
    subprocess.run([FF,'-y','-loglevel','error','-f','concat','-safe','0','-i',p+'/list.txt','-vf',f'fps=30,scale={w}:-2:flags=lanczos,format=yuv420p','-c:v','libx264','-crf','12','-preset','fast',f'{S}/clips4/{d}.mp4'],check=True)
